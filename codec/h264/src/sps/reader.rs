...
impl<R: BitRead> BitwiseReadFrom<R> for ProfileIdcRelated {
    ...
    fn read_from(reader: &mut R) -> Result<Self, Self::Error> {
        let chroma_format_idc = read_ue(reader)?.to_u8().unwrap().try_into()?;
        let separate_colour_plane_flag = if matches!(chroma_format_idc, ChromaFormatIdc::Chroma444)
        {
            Some(reader.read_bit()?)
        } else {
            None
        };
        let bit_depth_luma_minus8 = read_ue(reader)?;
        let bit_depth_chroma_minus8 = read_ue(reader)?;
        ...
        let seq_scaling_matrix = if seq_scaling_matrix_present_flag {
            let cnt = if !matches!(chroma_format_idc, ChromaFormatIdc::Chroma444) {
                8
            } else {
                12
            };
...
impl<R: BitRead> BitwiseReadFrom<R> for PicOrderCntType1 {
    ...
    fn read_from(reader: &mut R) -> Result<Self, Self::Error> {
        ...
        let offset_for_non_ref_pic = read_se(reader)?;
        let offset_for_top_to_bottom_field = read_se(reader)?;
...
impl<R: BitRead> BitwiseReadFrom<R> for FrameCropping {
    type Error = H264CodecError;
    fn read_from(reader: &mut R) -> Result<Self, Self::Error> {
        let frame_crop_left_offset = read_ue(reader)?;
        let frame_crop_right_offset = read_ue(reader)?;
        let frame_crop_top_offset = read_ue(reader)?;
        let frame_crop_bottom_offset = read_ue(reader)?;
        Ok(Self {
...
impl<R: BitRead> BitwiseReadFrom<R> for Sps {
    ...
    fn read_from(reader: &mut R) -> Result<Self, Self::Error> {
        ...
        {
            Some(ProfileIdcRelated::read_from(reader)?)
        } else {
            None
        };
        let log2_max_frame_num_minus4 = read_ue(reader)?;
        ...
        let pic_order_cnt_type_1 = if pic_order_cnt_type == 1 {
            Some(PicOrderCntType1::read_from(reader)?)
        } else {
            None
        };
