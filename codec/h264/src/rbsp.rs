...
pub struct RbspReader<R: Buf> {
    reader: bitstream_io::BitReader<R, bitstream_io::BigEndian>,
    buf_length: usize,
}

impl<'a> RbspReader<std::io::Cursor<&'a [u8]>> {
...
impl RbspReadExt for RbspReader<std::io::Cursor<&[u8]>> {
...
impl BitRead for RbspReader<std::io::Cursor<&[u8]>> {
    ...
    fn parse_with<'a, F: bitstream_io::FromBitStreamWith<'a>>(
        &mut self,
        context: &F::Context,
    ) -> Result<F, F::Error> {
