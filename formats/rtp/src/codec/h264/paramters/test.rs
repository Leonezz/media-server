...
mod tests {
    ...
    fn test_simple() {
        ...
        let parsed: RtpH264Fmtp = parameters.parse().unwrap();
        let bytes: [u8; 3] = parsed.profile_level_id.unwrap().into();
        assert_eq!(bytes, [0x42, 0xe0, 0x16]);
    ...
    fn test_simple2() {
        ...
        let parsed: RtpH264Fmtp = parameters.parse().unwrap();
        let bytes: [u8; 3] = parsed.profile_level_id.unwrap().into();
        assert_eq!(bytes, [0x42, 0xa0, 0x1e]);
        assert_eq!(
